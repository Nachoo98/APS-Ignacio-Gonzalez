...
# ================================================================
# Diseño filtro FIR con firwin2
# PREGUNTAS:
#   Con numtaps=30 (par)  => Tipo II => tiene cero topológico en Nyquist
#   Con numtaps=31 (impar) => Tipo I  => no tiene cero en Nyquist
#   Tipo II: retardo = (N-1)/2 no entero => demora no entera
# ================================================================
numtaps = 300
...
b_win = signal.firwin2(numtaps, freq=np.array([0., ws1, wp1, wp2, ws2, fs//2]),
                       gain=gains, nfreqs=2**14, window='flattop', fs=fs)
...
# Respuesta FIR
w_fir, h_fir = signal.freqz(b_win, worN=ww, fs=fs)
phase_fir     = np.unwrap(np.angle(h_fir))

# ================================================================
# Plot 1: Módulo (veces) y Fase — FIR
# ================================================================
fig, ax1 = plt.subplots(tight_layout=True)
ax1.set_title("Respuesta en Frecuencia - FIR (firwin2)")
ax1.plot(w_fir, abs(h_fir), 'C0', label='Módulo')
...
# ================================================================
# Plot 2: dB + Fase desenvuelta — FIR
# ================================================================
fig, ax1 = plt.subplots(tight_layout=True)
ax1.set_title('Respuesta en Frecuencia Digital (Escala dB) - FIR')
ax1.plot(w_fir, 20 * np.log10(abs(h_fir) + 1e-12), 'b')
...
# ================================================================
# Plot 3: Polos y Ceros (FIR, tf2zpk) + Retardo de Grupo
# ================================================================
fig_sys = plt.figure(figsize=(12, 5), tight_layout=True)

ax_z = fig_sys.add_subplot(1, 2, 1)
ceros, polos, k = signal.tf2zpk(b_win, [1])
...
ax_gd.plot(ww, gd_fir, 'm', linewidth=2)
ax_gd.set_title('Retardo de Grupo (Group Delay) - FIR')
...
# ================================================================
# Plot 4: Plantilla sombrada con respuesta FIR
# ================================================================
fig, ax1 = plt.subplots(figsize=(12, 5), tight_layout=True)
ax1.set_title("Frequency Response of FIR Filter (firwin2)")
ax1.plot(w_fir, 20 * np.log10(np.abs(h_fir) + 1e-12), 'b', linewidth=1.8, label='FIR (firwin2)')
...
# ================================================================
# Carga y filtrado del ECG (IIR + FIR)
# ================================================================
fs_ecg = 1000
...
ECG_f_fir    = signal.filtfilt(b_win, [1],    ecg_one_lead)
